...
- Should this be a first greeting, a light professional greeting, or a reconnect greeting?
- If the contact replies, when should a deeper follow-up happen?

Core objective:

TIME + REGION + PEOPLE + RELATIONSHIP + CHANNEL + OPPORTUNITY + FESTIVAL / CALENDAR CONTEXT -> COMMUNICATION ACTION
...
WHO x WHERE x WHEN x HOW x WHY NOW -> ACTION
...
new connection -> first introduction -> identity / background / current focus -> permission-safe context -> conversation.
...
existing connection -> festival / calendar timing -> light greeting -> reply or no reply -> optional follow-up.

Lifecycle:

Self Introduction -> Conversation -> Follow-up -> Festival / Periodic Touchpoint -> Reconnect -> DD / Coffee Chat / Collaboration / Recruiting -> Continued Relationship.
...
### Professional
Suitable for colleagues, recruiters, alumni, institutional contacts, managers and professional acquaintances.
...
### Reconnect
Suitable for dormant contacts, prior informal meetings, previous recruiters and professional contacts not spoken to recently.
...
- region-specific Hong Kong / Macau public holidays
...
Each festival/holiday needs a greeting_appropriate flag.
...
Optional:
- Boxing Day
- region-specific England / Wales / Scotland / Northern Ireland differences

Again, not every bank holiday should automatically trigger outreach.

### 5.5 Other Regional Calendars

Architecture must remain extensible for Hong Kong, Singapore, Europe, Australia, Middle East and country-specific cultural holidays.
...
Do not commit real names, private contact data, confidential job information, or employer-specific sensitive information.
...
Festival Greeting -> Reply -> Conversation -> Context Update -> Follow-up Timing -> DD / Coffee Chat / Career Update / Collaboration / Recruiting -> Human Decision.
...
3. Follow-up timing should consider reply timestamp, timezone, working window, relationship class, prior follow-up frequency, due date, opportunity urgency and channel.
...
PTime consumes only a reference / template ID, not private PCV content unless explicitly supplied.
...
Greeting copy should be configuration.
...
Do not infer religion, ethnicity, nationality, political identity or sensitive personal traits from contact location alone.

Use explicit region metadata, explicit user-selected festival applicability, explicit contact preference and known professional context.
...
PTime V2.1:
adds cultural / festival / follow-up timing.
...
PCV Self-Introduction = first-contact communication asset.
PTime General Greeting = relationship-maintenance timing asset.
...
- do not combine holiday greeting with aggressive referral ask
...
Do NOT bump VERSION, pyproject.toml, ptime.__version__ or VERSION.md current release until implementation and tests are complete.
...
TIME + TIMEZONE + FOLLOW-UP STATE + FESTIVAL / CULTURAL TIMING + RELATIONSHIP -> NEXT BEST COMMUNICATION WINDOW
...
The goal is to communicate at the right time, for the right reason, with the right level of intensity.
